...
CNode* ConnectNode(CAddress addrConnect, const char *pszDest, int64 nTimeout)
{
    ...
    /// debug print
    if (fDebugNet) printf("trying connection %s lastseen=%.1fhrs\n",
    ...
    if (pszDest ? ConnectSocketByName(addrConnect, hSocket, pszDest, GetDefaultPort()) : ConnectSocket(addrConnect, hSocket))
    {
        ...
        /// debug print
        if (fDebugNet) printf("connected %s\n", pszDest ? pszDest : addrConnect.ToString().c_str());
...
void CNode::CloseSocketDisconnect()
{
    ...
    {
        if (fDebugNet) printf("disconnecting node %s\n", addrName.c_str());
        closesocket(hSocket);
...
void ThreadSocketHandler2(void* parg)
{
    ...
    while (true)
    {
        ...
        if (hListenSocket != INVALID_SOCKET && FD_ISSET(hListenSocket, &fdsetRecv))
        {
            ...
            {
                if (fDebugNet) printf("accepted connection %s\n", addr.ToString().c_str());
...
void DumpAddresses()
{
    ...
    if (fDebugNet) printf("Flushed %d addresses to peers.dat  %"PRI64d"ms\n",
...
void ThreadDumpAddress2(void* parg)
{
    ...
    while (!fShutdown)
    {
        DumpAddresses();
        vnThreadsRunning[THREAD_DUMPADDRESS]--;
        Sleep(900000);        // Avoid frequent address dumps
        vnThreadsRunning[THREAD_DUMPADDRESS]++;
    }
    vnThreadsRunning[THREAD_DUMPADDRESS]--;
}
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (addrBind.IsIPv6()) {
#ifdef IPV6_V6ONLY
    // fix invalid conversion error when compiling on Windows
#ifdef WIN32
